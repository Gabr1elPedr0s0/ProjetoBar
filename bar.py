...
def verifica_sessao():
    if "login" in session and session["login"]:
        ...
        return False  # Fixed: Return False when the session is not valid

# CONEXÃO COM O BANCO DE DADOS 
def conecta_database():
    conexao = sql.connect("db_bar.db")
...
def iniciar_db():
    ...
    with app.open_resource('esquema.sql', mode='r') as comandos:  # Fixed: open_instance_resource to open_resource
...
def cadastro():
    if verifica_sessao():
        ...
        desc_prod = request.form['desc_prod']
        preco_prod = request.form['preco_prod']
        img_prod = request.files['img_prod']  # Fixed: Corrected the way to get the file from the form
        id_foto = str(uuid.uuid4().hex)
        filename = id_foto + nome_prod + '.png'
        img_prod.save("static/img/produtos/" + filename)
        conexao = conecta_database()
        conexao.execute('INSERT INTO produtos (nome_prod, desc_prod, preco_prod, img_prod) VALUES (?, ?, ?, ?)',
                        (nome_prod, desc_prod, preco_prod, filename))  # Fixed: Added a comma to separate values
        conexao.commit()
        conexao.close()
        return redirect("/adm")
    else:
